...
main(int argc, char *argv[]) {
    ...
    try {
        ...
        cin >> currMessage;
        bool isFailure = true;
        while(currMessage != "quit") {

            const char* currMessageArr = currMessage.c_str();
           
            c150debug->printf(C150APPLICATION,"%s: Writing message: \"%s\"", argv[0], currMessageArr);
            
            sock -> write(currMessageArr, strlen(currMessageArr)+1); // +1 includes the null
            ...
            if (sock -> timedout()) {
                isFailure = true;
                int i = 0;
                while (i < 5 and isFailure) {
                        readlen = sock -> read(incomingMessage, sizeof(incomingMessage));
                        isFailure = sock -> timedout();
                        i++;
                    }
                if (i == 5) {
